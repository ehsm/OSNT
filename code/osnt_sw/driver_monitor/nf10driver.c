...
struct dentry *file_rx_dne;
struct dentry *file_rx_buff;
...
static int __devinit nf10_probe(struct pci_dev *pdev, const struct pci_device_id *id){
    ...
    *(((uint64_t*)card->cfg_addr)+26) = 0; // disable RX interrupts
    ...
    card->rx_buff_ptr = (uint64_t)dma_alloc_coherent(&pdev->dev, card->rx_buff_mask+1+PAGE_SIZE, &(card->rx_buff_physical_addr), GFP_KERNEL);

    rx_dne_ptr = (void*)card->host_rx_dne_ptr;
    rx_buff_ptr = (void*)card->rx_buff_ptr;

    if( (card->host_rx_dne_ptr == NULL) ||
        (card->host_tx_dne_ptr == NULL) ||
        (card->rx_buff_ptr == 0) ||
        (((card->rx_buff_ptr) & 0xfffULL) != 0)){
 ...
 err_out_free_private2:
    ...
    dma_free_coherent(&pdev->dev, card->rx_buff_mask+1, (void*)card->rx_buff_ptr, card->rx_buff_physical_addr);
 err_out_iounmap:
...
static void __devexit nf10_remove(struct pci_dev *pdev){
    ...
    if(card){
        *(((uint64_t*)card->cfg_addr)+44) = 0;
        ...
        pci_free_consistent(pdev, card->tx_dne_mask+1, card->host_tx_dne_ptr, card->host_tx_dne_dma);
        dma_free_coherent(&pdev->dev, card->rx_dne_mask+1, card->host_rx_dne_ptr, card->host_rx_dne_dma);
        dma_free_coherent(&pdev->dev, card->rx_buff_mask+1+PAGE_SIZE, (void*)card->rx_buff_ptr, card->rx_buff_physical_addr);
...
static int nf10_rx_dne_mmap(struct file *filp, struct vm_area_struct *vma)
{
    if (remap_pfn_range(vma, vma->vm_start, (virt_to_phys(rx_dne_ptr)>>PAGE_SHIFT),
...
static int nf10_rx_buff_mmap(struct file *filp, struct vm_area_struct *vma)
{
    if (remap_pfn_range(vma, vma->vm_start, (virt_to_phys(rx_buff_ptr)>>PAGE_SHIFT),
        vma->vm_end - vma->vm_start,
        vma->vm_page_prot))
        return -EAGAIN;
...
static const struct file_operations nf10_rx_dne_mmap_fops = {
	.mmap = nf10_rx_dne_mmap,
};

static const struct file_operations nf10_rx_buff_mmap_fops = {
	.mmap = nf10_rx_buff_mmap,
};

static int __init nf10_init(void)
{
	printk(KERN_INFO "nf10: module loaded\n");
	file_rx_dne = debugfs_create_file("nf10_rx_dne_mmap", 0644, NULL, NULL, &nf10_rx_dne_mmap_fops);
	file_rx_buff = debugfs_create_file("nf10_rx_buff_mmap", 0644, NULL, NULL, &nf10_rx_buff_mmap_fops);
...
static void __exit nf10_exit(void)
{
    ...
    debugfs_remove(file_rx_dne);
    debugfs_remove(file_rx_buff);
