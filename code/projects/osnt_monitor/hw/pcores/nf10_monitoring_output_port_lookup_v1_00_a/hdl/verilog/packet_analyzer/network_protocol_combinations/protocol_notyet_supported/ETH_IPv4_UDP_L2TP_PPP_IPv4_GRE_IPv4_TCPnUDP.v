...
////////////////////////////  Includes & Defines //////////////////////////////
	 ...
	 module ETH_IPv4_UDP_L2TP_PPP_IPv4_GRE_IPv4_TCPnUDP 
 ...
 always @ (posedge clk or posedge reset)
	 ...
	 begin  
			...
			pkt_ip_hdr_len_w_0 = pkt_ip_hdr_len_0;
	 		...
	 		pkt_ip_hdr_len_w_2 = pkt_ip_hdr_len_2;
	 				...
	 				begin
	 					...
	 					begin
	 						...
	 						begin
	 							...
	 							if( !pkt_flags_w[`PKT_FLG_FRG] )
			 					begin
			 						pkt_flags_w[`PKT_FLG_GRE] = (in_data_d0[7:0] == `IP_GRE);
	 		...
	 		begin		 	
	 			...
	 			begin
	 				...
	 				begin
	 					...
	 					4'd5:
	 					begin
	 						case(in_data_d0[47:44])
		 					4'b0000: //{checksum = 0, routing = 0, key = 0, sequence no = 0}						
		 					begin
		 						pkt_gre_hdr_len_w = 4'b0;
	 							pkt_flags_w[`PKT_FLG_IPv4] =(in_data_d0[31:16] ==`GRE_IP) ;
	 							if(pkt_flags_w[`PKT_FLG_IPv4])
	 							begin				
	 								pkt_ip_hdr_len_w_2 = in_data_d0[11:8];
									if (in_eop_d0) /*small pkt*/
									begin
										nxt_state = IDLE;
									end
	 			...
	 			begin
	 				...
	 				begin
	 					...
	 					begin
		 					...
		 					begin
		 						pkt_flags_w[`PKT_FLG_FRG] = (in_data_d0[93] || (in_data_d0[92:80] != 14'd0));
			 					pkt_flags_w[`PKT_FLG_TCP] = (in_data_d0[71:64] == `IP_TCP);
			 					pkt_flags_w[`PKT_FLG_UDP] = (in_data_d0[71:64] == `IP_UDP);
	 							pkt_src_ip_w = in_data_d0[47:16];
	 							pkt_dst_ip_w = {in_data_d0[15:0],pkt_dst_ip_w[15:0]};
								if (in_eop_d0) /*small pkt*/
								begin
									nxt_state = IDLE;
								end
	 			...
	 			begin
	 				...
	 				begin
	 					...
	 					begin
		 					...
		 					4'b0000: 						
		 					begin
		 						pkt_dst_ip_w = {pkt_dst_ip_w[31:16],in_data_d0[127:112]};
	 							...
	 							begin
			 						...
			 						begin
	 									...
	 									4'd5:
	 									begin
			 								pkt_src_port_w = in_data_d0[111:96];
			 								pkt_dst_port_w = in_data_d0[95:80];
			 								
			 								if (pkt_flags[`PKT_FLG_UDP])
						 					begin
						 						pkt_flags_w[`PKT_FLG_RTP] = (in_data_d0[47:44] == `UDP_RTP);
												if (in_eop_d0) /*small pkt*/
												begin
													pkt_valid_w = 1'b1;
													nxt_state = IDLE;
			 								...
			 								begin
												pkt_tcp_hdr_len_w = in_data_d0[15:12]; // in 32 bit words
			 									if (in_eop_d0) /*small pkt*/
												begin
													pkt_valid_w = 1'b1;
													nxt_state = IDLE;
	 		...
	 		begin		 	
	 			...
	 			begin
			 		...
			 		4'd5:
			 		begin	 
			 			pkt_flags_w[`PKT_FLG_GET] = (in_data_d0[79:56] == `TCP_GET);
			 			pkt_flags_w[`PKT_FLG_POST] = (in_data_d0[79:48] == `TCP_POST);
	 				...
	 				begin
	 					...
	 					begin
	 						...
	 						begin
	 							...
	 							if(!pkt_flags_w[`PKT_FLG_FRG] )
			 					begin
			 						pkt_flags_w[`PKT_FLG_GRE] = (in_data_d0[23:16] == `IP_GRE);
	 								if(pkt_flags_w[`PKT_FLG_GRE])
	 								begin
	 									if (in_eop_d0) /*small pkt*/
										begin
	 		...
	 		begin		 	
	 			...
	 			begin
	 				...
	 				begin
	 					...
	 					4'd5:
	 					begin
	 						case(in_data_d0[63:60])
		 					4'b0000: //{checksum = 0, routing = 0, key = 0, sequence no = 0}						
		 					begin
		 						pkt_gre_hdr_len_w = 4'b0;
	 							pkt_flags_w[`PKT_FLG_IPv4] =(in_data_d0[47:32] ==`GRE_IP) ;
	 							if(pkt_flags_w[`PKT_FLG_IPv4])
	 							begin				
	 								pkt_ip_hdr_len_w_2 = in_data_d0[27:24];
	 								if (in_eop_d0) /*small pkt*/
									begin
										nxt_state = IDLE;
									end
	 			...
	 			begin
	 				...
	 				begin
	 					...
	 					begin
		 					...
		 					4'b0000: 						
		 					begin
		 						pkt_flags_w[`PKT_FLG_FRG] = (in_data_d0[109] || (in_data_d0[108:96] != 14'd0));
			 					pkt_flags_w[`PKT_FLG_TCP] = (in_data_d0[87:80] == `IP_TCP);
			 					pkt_flags_w[`PKT_FLG_UDP] = (in_data_d0[87:80] == `IP_UDP);
	 							pkt_src_ip_w = in_data_d0[63:32];
	 							pkt_dst_ip_w = in_data_d0[31:0];
		 						if(!pkt_flags_w[`PKT_FLG_FRG])
		 						begin
		 							if (in_eop_d0) /*small pkt*/
									begin
	 					...
	 					begin
		 					...
		 					begin
	 							...
	 							begin
		 							...
		 							4'd5:
									begin
			 							pkt_src_port_w = in_data_d0[127:112];
			 							pkt_dst_port_w = in_data_d0[111:96];
			 							
			 							if (pkt_flags[`PKT_FLG_UDP])
					 					begin
					 						pkt_flags_w[`PKT_FLG_RTP] = (in_data_d0[63:60] == `UDP_RTP);
											if (in_eop_d0) /*small pkt*/
											begin
												pkt_valid_w = 1'b1;
												nxt_state = IDLE;
		 								...
		 								begin
											pkt_tcp_hdr_len_w = in_data_d0[31:28]; // in 32 bit words
						 					if (in_eop_d0) /*small pkt*/
											begin
												pkt_valid_w = 1'b1;
												nxt_state = IDLE;
	 		...
	 		begin		 	
	 			...
	 			begin
					case(pkt_tcp_hdr_len)
			 		4'd5:
			 		begin	 
			 			pkt_flags_w[`PKT_FLG_GET] = (in_data_d0[95:72] == `TCP_GET);
			 			pkt_flags_w[`PKT_FLG_POST] = (in_data_d0[95:64] == `TCP_POST);
	 ...
	 assign pkt_attributes[(`PRTCL_ID_WIDTH+`PRTCL_ID_OFFSET)-1:`PRTCL_ID_OFFSET] = `PRIORITY_ETH_IPv4_UDP_L2TP_PPP_IPv4_GRE_IPv4_TCPnUDP;           
