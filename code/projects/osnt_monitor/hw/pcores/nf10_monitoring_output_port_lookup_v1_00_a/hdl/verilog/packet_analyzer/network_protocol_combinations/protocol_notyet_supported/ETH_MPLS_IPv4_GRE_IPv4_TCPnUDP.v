...
////////////////////////////  Includes & Defines //////////////////////////////
	 ...
	 module ETH_MPLS_IPv4_GRE_IPv4_TCPnUDP 
 ...
 always @ (posedge clk or posedge reset)
	 ...
	 begin  
	 		...
	 		begin		
	 			...
	 			begin
	 				...
	 				begin
		 				...
		 				begin
		 					pkt_gre_hdr_len_w = 4'b0;
	 						pkt_flags_w[`PKT_FLG_IPv4] = (in_data_d0[63:48] == `GRE_IP);
	 						if(pkt_flags_w[`PKT_FLG_IPv4])
	 						begin
	 			...
	 			begin
	 				...
	 				begin
		 				...
		 				begin
			 				...
			 				pkt_flags_w[`PKT_FLG_TCP] = (in_data_d0[103:96] == `IP_TCP);
	 						pkt_flags_w[`PKT_FLG_UDP] = (in_data_d0[103:96] == `IP_UDP);
	 						pkt_src_ip_w = in_data_d0[79:48];
		 					pkt_dst_ip_w = in_data_d0[47:16];
			 				if(!pkt_flags_w[`PKT_FLG_FRG])
			 				begin	
			 					...
			 					begin
	 								...
	 								4'd5:
	 								begin		
	 									pkt_src_port_w = in_data_d0[15:0];
			 						end
			 						default:
			 						begin
			 							pkt_src_port_w = {`PORT_WIDTH{1'b0}};
	 			...
	 			begin
	 				...
	 				begin
		 				...
		 				begin
	 						...
	 						4'd5:
	 						begin		
	 							pkt_dst_port_w = in_data_d0[127:112];
	 							
	 							if (pkt_flags[`PKT_FLG_UDP])
			 					begin
			 						pkt_flags_w[`PKT_FLG_RTP] = (in_data_d0[79:76] == `UDP_RTP);
									if (in_eop_d0) /*small pkt*/
									begin
										pkt_valid_w = 1'b1;
										nxt_state = IDLE;
			 					...
			 					begin
			 						pkt_tcp_hdr_len_w = in_data_d0[47:44]; // in 32 bit words
									if (in_eop_d0) /*small pkt*/
									begin
										pkt_valid_w = 1'b1;
										nxt_state = IDLE;
	 		...
	 		begin		 	
	 			...
	 			begin
					...
					4'd5:
					begin	 
						pkt_flags_w[`PKT_FLG_GET] = (in_data_d0[111:88] == `TCP_GET);
						pkt_flags_w[`PKT_FLG_POST] = (in_data_d0[111:80] == `TCP_POST);
	 ...
	 assign pkt_attributes[(`PRTCL_ID_WIDTH+`PRTCL_ID_OFFSET)-1:`PRTCL_ID_OFFSET] = `PRIORITY_ETH_MPLS_IPv4_GRE_IPv4_TCPnUDP;           
