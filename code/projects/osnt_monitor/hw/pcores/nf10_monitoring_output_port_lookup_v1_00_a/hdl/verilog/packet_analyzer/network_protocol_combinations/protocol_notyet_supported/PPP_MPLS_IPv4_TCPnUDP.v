...
////////////////////////////  Includes & Defines //////////////////////////////
   ...
   reg [`NUM_INPUT_QUEUES-1:0] 	pkt_input_if;	 
	 ...
	 begin  
	 		...
	 		begin		 	
	 			...
	 			begin
		 			...
		 			begin
		 				pkt_flags_w[`PKT_FLG_MPLS] = (in_data_d0[111:96] == `PPP_MPLS_UNICAST);
			 			if (pkt_flags_w[`PKT_FLG_MPLS])
			 			begin
			 				if (in_data_d0[72] == 1'b1)/*MPLS bottom of label stack bit*/
		 					begin
		 						pkt_flags_w[`PKT_FLG_IPv4] = ( (in_data_d0[63:60] == `IP_VER4)/*Version = 4*/ 
		 																 	 &&(in_data_d0[59:56]	== 	4'd5)/*Header Length = 5*/);	
		 						if(pkt_flags_w[`PKT_FLG_IPv4])
		 						begin
		 							pkt_ip_hdr_len_w = in_data_d0[59:56];
				 					pkt_flags_w[`PKT_FLG_FRG] = (in_data_d0[13] || (in_data_d0[12:0] != 14'd0)); /* @ shahbaz - resolve */
	 		...
	 		begin		
	 			if (in_wr_d0)
	 			begin	 				
	 				pkt_flags_w[`PKT_FLG_TCP] = (in_data_d0[119:112] == `IP_TCP);
	 				pkt_flags_w[`PKT_FLG_UDP] = (in_data_d0[119:112] == `IP_UDP);
	 				pkt_src_ip_w = in_data_d0[95:64];
 					pkt_dst_ip_w = in_data_d0[63:32];
	 				...
	 				begin	 				
		 				...
		 				begin	 				
			 				...
			 				4'd5:
			 				begin
			 					pkt_src_port_w = in_data_d0[31:16];
			 					pkt_dst_port_w = in_data_d0[15:0];
			 					nxt_state = PKT_WORD2;
			 				end
			 				default:
			 				begin
	 		...
	 		begin		
	 			...
	 			begin
					...
					begin
						pkt_flags_w[`PKT_FLG_RTP] = (in_data_d0[95:92] == `UDP_RTP);
						nxt_state = PKT_WAIT_HDR;	
					end
	 				else if (pkt_flags[`PKT_FLG_TCP])
	 				begin
	 					pkt_tcp_hdr_len_w = in_data_d0[63:60]; // in 32 bit words
			 			nxt_state = PKT_WORD3;
			 		end
					else
						nxt_state = PKT_WAIT_HDR;	
			...
			PKT_WORD3:
	 		begin		
	 			...
	 			begin
				 	...
				 	4'd5:
				 	begin	 
				 		pkt_flags_w[`PKT_FLG_GET] = (in_data_d0[127:104] == `TCP_GET);
			 			pkt_flags_w[`PKT_FLG_POST] = (in_data_d0[127:96] == `TCP_POST);
	 ...
	 assign pkt_attributes[(`PRTCL_ID_WIDTH+`PRTCL_ID_OFFSET)-1:`PRTCL_ID_OFFSET] = `PRIORITY_PPP_MPLS_IPv4_TCPnUDP;           
